...
IEnumerable<(string declaration, string run1, string run2)> Generate(int depth)
{
    ...
    static string Body1(string suffix, bool hasEntity, bool hasMessage, IEnumerable<string> generics, IEnumerable<string> constraints)
    {
        ...
        return
$@"public static Runner RunEach<{string.Join(", ", generics)}>({runEach} run, params Filter[] filters) {string.Join(" ", constraints)} => RunEach(
    Filter.All<{string.Join(", ", generics)}>(filters),
    (in {react} message, in Segment segment) =>
    {{
...
$@"public static Runner RunEach<{string.Join(", ", generics)}>({runEach} run, params Filter[] filters) {string.Join(" ", constraints)} => RunEach(
    Filter.All<{string.Join(", ", generics)}>(filters),
    (in {react} react, in {receive} receive, in Segment segment) =>
    {{
        var (entities, count) = segment.Entities;
        {string.Join(" ", storeVars)}
        {forRun}
