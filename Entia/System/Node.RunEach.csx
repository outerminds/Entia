...
IEnumerable<(string declaration, string run)> Generate(int depth)
{
    ...
    static IEnumerable<(string declaration, string run)> Declaration(bool hasPhase, bool hasEntity, string[] generics)
    {
        ...
        if (hasEntity || generics.Length > 0)
        {
            yield return (
...
$@"public static Node RunEach{runGenerics}({runStoreType} run, Filter? filter = null) {runConstraints} => RunEach(
    ...
    {filter}, {dependencies}).Name(run.Method);");
        }
...
$@"public static Node RunEach{runGenerics}({runComponentType} run, Filter? filter = null) {runConstraints} => RunEach(
    ...
    for (int i = 0; i <= depth; i++)
    {
        ...
        foreach (var pair in Declaration(true, true, generics)) yield return pair;
        foreach (var pair in Declaration(true, false, generics)) yield return pair;
        foreach (var pair in Declaration(false, true, generics)) yield return pair;
        foreach (var pair in Declaration(false, false, generics)) yield return pair;
    }
}
