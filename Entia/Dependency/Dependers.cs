		...
		public IDependency[] Dependencies(MemberInfo member)
		{
			...
			IEnumerable<IDependency> Next(MemberInfo current)
			{
				...
				if (set.Add(current) && attributes.OfType<IgnoreAttribute>().None())
				{
					...
					{
						case Type type:
							foreach (var dependency in type.Hierarchy()
								.SelectMany(child => child.GetCustomAttributes(true)
									.SelectMany(attribute => Get(attribute.GetType())
										.Depend(attribute, current, _world))))
								yield return dependency;
							break;
						default:
							foreach (var dependency in attributes.SelectMany(attribute => Get(attribute.GetType()).Depend(attribute, current, _world)))
								yield return dependency;
							break;
					}
					...
					{
						case Type type:
							foreach (var dependency in Get(type).Depend(type, current, _world)) yield return dependency;
							break;
